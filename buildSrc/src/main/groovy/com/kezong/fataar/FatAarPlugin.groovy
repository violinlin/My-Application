...
class FatAarPlugin implements Plugin<Project> {
    ...
    private void doAfterEvaluate() {
        ...
        project.android.libraryVariants.all { LibraryVariant variant ->
            ...
            project.delete(DirectoryManager.getCopyAArDirectory())

            if (!artifacts.isEmpty()) {
                FatUtils.logAnytime("copy aars:--------------------------------------------")
                for (ResolvedArtifact item: artifacts) {
                    FatUtils.logAnytime("module:"+item.getModuleVersion().toString() + " type:" + item.getType() + " name:"+item.getName() + " path:" +item.getFile().getParent())
                    if (!item.getModuleVersion().toString().startsWith('androidx.')) {
                        project.copy {
                            from item.getFile()
                            into DirectoryManager.getCopyAArDirectory()
                        }
                    }
                }
